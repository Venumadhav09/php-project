pipeline {
    ...
    stages{
        ...
        stage('Build docker image'){
            steps{
                script{
                    sh 'docker build -t venumadhav09/venuimg:v1 .'
                    sh 'docker images'
                }
            }
        }
          stage('Docker login') {
            steps {
                withCredentials([usernamePassword(credentialsId: 'dockerhub-pwd', passwordVariable: 'PASS', usernameVariable: 'USER')]) {
                    ...
                    sh 'docker push venumadhav09/venuimg:v1'
                }
            }
        }
            ...
            steps {
               script {
                   def dockerrm = 'sudo docker rm -f My-first-containe2211 || true'
                    def dockerCmd = 'sudo docker run -itd --name My-first-containe2211 -p 9090:80 venumadhav09/venuimg:v1'
                    sshagent(['sshkeypair']) {
